...
void THTensor_(random)(THTensor *self, at::Generator *_generator)
{
  ...
  TH_TENSOR_APPLY(scalar_t, self, *self_data = (uint64_t)(cpu_engine() % (LONG_MAX + 1ULL)););
#elif defined(TH_REAL_IS_FLOAT)
  TH_TENSOR_APPLY(scalar_t, self, *self_data = (float)(((uint32_t)(cpu_engine())) % ((1ULL << FLT_MANT_DIG) + 1)););
#elif defined(TH_REAL_IS_DOUBLE)
  TH_TENSOR_APPLY(scalar_t, self, *self_data = (double)(cpu_engine() % ((1ULL << DBL_MANT_DIG) + 1)););
#else
#error "Unknown type"
#endif
...
#endif
    TH_TENSOR_APPLY(scalar_t, self, *self_data = static_cast<scalar_t>(static_cast<int64_t>((((uint32_t)(cpu_engine())) % range) + min));)
}
...
{
  auto& cpu_engine = _generator->getCPUEngine();
  static std::geometric_distribution<> geometric(p);
...
void THTensor_(uniform)(THTensor *self, at::Generator *_generator, double a, double b)
{
  auto& cpu_engine = _generator->getCPUEngine();
  #if defined(TH_REAL_IS_FLOAT)
  static std::uniform_real_distribution<float> uniform((float)a, (float)b);
...
void THTensor_(normal)(THTensor *self, at::Generator *_generator, double mean, double stddev)
{
  ...
  if (size >= 16 && THTensor_(isContiguous)(self)) {
    ...
    TH_TENSOR_APPLY(scalar_t, self, *self_data = (scalar_t)normal(cpu_engine););
  }
}
...
{
  auto& cpu_engine = _generator->getCPUEngine();
  static std::exponential_distribution<double> exponential(lambda);
...
{
  auto& cpu_engine = _generator->getCPUEngine();
  static std::cauchy_distribution<double> cauchy(median, sigma);
...
{
  auto& cpu_engine = _generator->getCPUEngine();
  static std::lognormal_distribution<double> logNormal(mean, stdv);
  TH_TENSOR_APPLY(scalar_t, self, *self_data = (scalar_t)logNormal(cpu_engine););
}
...
void THTensor_(multinomialAliasDraw)(THLongTensor *self, at::Generator *_generator, THLongTensor *J, THTensor *q)
{
  ...
  for (i=0; i < output_nelem; i++)
    {
      static std::uniform_real_distribution<double> uniform(0, K);
      rand_ind = uniform(cpu_engine);

      _q = THTensor_(fastGet1d)(q, rand_ind);
      static std::bernoulli_distribution bernoulli(_q);
