...
variable_list NormBackward2::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = norm_backward(grad, self.to(grad.type().scalarType()), p, result).to(self.type().scalarType());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
variable_list NormBackward3::apply(variable_list&& grads) {
  ...
  auto result = result_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = norm_backward(grad, self.to(grad.type().scalarType()), p, result, dim, keepdim).to(self.type().scalarType());
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
...
variable_list SlogdetBackward::apply(variable_list&& grads) {
  ...
  auto result1 = result1_.unpack(shared_from_this());
  if (should_compute_output({ self_ix })) {
    auto grad_result = slogdet_backward(grad, self, result0, result1);
    copy_range(grad_inputs, self_ix, grad_result);
  }
  return grad_inputs;
}
