...
RegisterOperators reg({
  ...
  Operator(
      "aten::_th_masked_scatter_bool_(Tensor(a!) self, Tensor mask, Tensor source) -> Tensor(a!)",
      [](Stack & stack) {
          auto self = (std::move(peek(stack, 0, 3))).toTensor();
          auto result_ = at::_th_masked_scatter_bool_(
              self,
              (std::move(peek(stack, 1, 3))).toTensor(),
              (std::move(peek(stack, 2, 3))).toTensor()
          );
  ...
  Operator(
      "aten::_th_masked_select_bool(Tensor self, Tensor mask) -> Tensor",
      [](Stack & stack) {
      
          auto result_ = at::_th_masked_select_bool(
              (std::move(peek(stack, 0, 2))).toTensor(),
              (std::move(peek(stack, 1, 2))).toTensor()
          );
          drop(stack, 2);
